...
export function FormationVisualizer() {
  ...
  const [selectedSubFormation, setSelectedSubFormation] =
    useState<SubFormation | null>(null);
  const [formations, setFormations] = useState<Formation[]>([]);

  useEffect(() => {
    console.log("All Formations:", formationsData.FORMATIONS);
    console.log(
      "Number of formations loaded:",
      formationsData.FORMATIONS.length
    );

    // Log each formation's sub-formations
    formationsData.FORMATIONS.forEach((f, i) => {
      console.log(
        `Formation ${i + 1}: ${f.formation} has ${
          f["sub-formations"].length
        } sub-formations`
      );
    ...
    const parsedFormations: Formation[] = formationsData.FORMATIONS.map(
      (f: any) => ({
  ...
  const getFormationName = (subForm: SubFormation): [string, string] => {
    // Find the key that is NOT description, shape, notes, or name
    const keys = Object.keys(subForm).filter(
      (k) => !["description", "shape", "notes", "name"].includes(k)
    );
  ...
  const getFormationIcon = (formation: string) => {
    if (formation.includes("5"))
      return <Shield className="h-4 w-4 text-blue-500" />;
    if (formation.includes("3-4-3") || formation.includes("4-3-3"))
      return <Swords className="h-4 w-4 text-amber-500" />;
    if (formation.includes("4-4-2"))
      return <Layers className="h-4 w-4 text-emerald-500" />;
    if (formation.includes("4-5-1"))
      return <ListTree className="h-4 w-4 text-purple-500" />;
      ...
      <Card className="lg:col-span-1 border-0 shadow-sm bg-gradient-to-b from-background to-muted/20">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-sm">
            Select a formation to explore tactical variations
          </CardDescription>
        </CardHeader>
                      ...
                      {formation["sub-formations"].map((subForm, subIndex) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Badge
                              variant="outline"
                              className={`ml-2 text-xs h-5 rounded ${getShapeColor(
                                subForm.shape
                              )}`}
                            >
                              {subForm.shape}
          ...
          {selectedSubFormation ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card className="border-secondary/20">
                  ...
                  <CardContent>
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-4 h-4 rounded-full ${getShapeColor(
                          selectedSubFormation.shape
                        )}`}
                      />
                      <span className="text-sm font-medium">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      This formation utilizes a{" "}
                      {selectedSubFormation.shape.toLowerCase()} structure for
                      optimal tactical positioning.
                    </p>
                  </CardContent>
                </Card>
              </div>
                ...
                <CardContent>
                  <div className="relative w-full max-w-md mx-auto bg-gradient-to-b from-green-400 to-green-500 rounded-lg overflow-hidden">
                    <div className="aspect-[2/3] relative border-4 border-white">
                      ...
                      {(() => {
                        // Get the formation name from the sub-formation object
                        const [formationName] = getFormationName(selectedSubFormation);
                        const positions = getFormationPositions(formationName);
                        
                        // Debug logging
                        console.log('=== FORMATION DEBUG ===');
                        console.log('Selected SubFormation:', selectedSubFormation);
                        console.log('Formation Name extracted:', formationName);
                        console.log('Formation Name type:', typeof formationName);
                          ...
                          return (
                            <div className="text-center py-8">
                              <div className="text-sm text-gray-500 dark:text-gray-400">
                                No position data found for formation: {formationName}
                              </div>
                              <div className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                                Check if this formation exists in formationPositions.ts
                              </div>
                            </div>
                          );
                        }
                        ...
                        return positions.map((pos, index) => (
                          ...
                          >
                            <div
                              className={`w-6 h-6 rounded-full ${getPositionColor(
                                pos.category
                              )} border-2 border-white flex items-center justify-center text-white text-xs font-bold shadow-lg`}
                            >
            ...
            <div className="text-center py-12 animate-bounce-in">
              ...
              <h3 className="text-lg font-semibold mb-2">
                {selectedFormation.formation} Formation Family
              </h3>
              <p className="text-muted-foreground mb-4">
                This formation has {selectedFormation["sub-formations"].length}{" "}
                tactical variations. Select a sub-formation to see detailed
                analysis.
              </p>
              <Badge variant="outline">
                {selectedFormation["sub-formations"].length} Sub-formations
                Available
              </Badge>
            </div>
          ) : (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                Select a formation from the list to explore its tactical
                variations and detailed analysis.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <details className="text-sm">
          ...
          <div className="mt-2 p-3 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">
            <h4 className="font-semibold mb-2">
              Formations Loaded: {formations.length}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
